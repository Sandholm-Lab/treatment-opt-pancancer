// Taken from
// https://raw.githubusercontent.com/cran/tmg/master/src/HmcSampler.cpp
...
MatrixXd HmcSampler::sampleNext(bool returnTrace) {
  MatrixXd tracePoints =
      MatrixXd(dim, 0);  // this matrix will only be filled if(returnTrace)
  ...
  while (2) {
    ...
    bool first_bounce =
        true;  // for the first move, we do not fear that a small t1 or t2 is
               // due to being in the boundary from the previous bounce.
    while (1) {
      t1 = 0;
      if (!linearConstraints.empty()) _getNextLinearHitTime(a, b, t1, cn1);
      ...
      if (t == 0 || tt < t) {  // if no wall to be hit (t==0) or not enough time
                               // left to hit the wall (tt<t2)
        break;
      } else {
        if (returnTrace) {
    ...
    if (velsign < 0) { /* cout << "wrong velocity " << endl; */
      continue;
    }  // go to beginning of while(2)

    VectorXd bb =
        sin(tt) * a +
        cos(tt) * b;  // make last move of time tt without hitting walls

    double check = _verifyConstraints(bb);
    if (check >= 0) {  // verify that we don't violate the constraints due to a
...
void HmcSampler::addQuadraticConstraint(const MatrixXd& A, const VectorXd& B,
                                        const double& C) {
...
void HmcSampler::_updateTrace(VectorXd const& a, VectorXd const& b,
                              double const& t, MatrixXd& tracePoints) {
